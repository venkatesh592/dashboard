...
export class ImageCropper extends ImageCropperModel {
    ...
    public customPrepare(canvas:HTMLCanvasElement) {
        ...
        let responsiveWidth:number = canvas.parentElement ? canvas.parentElement.clientWidth : 0;
        if (responsiveWidth > 0 && this.cropperSettings.dynamicSizing) {
            cropCanvas.width = responsiveWidth;
            buffer.width = responsiveWidth;
            canvas.width = responsiveWidth;
        } else {
    ...
    public customResizeCanvas(canvas:HTMLCanvasElement,width:number, height:number, setImage:boolean = false):void {
        canvas.width = this.cropCanvas.width = this.width = this.canvasWidth = this.buffer.width = width;
        canvas.height = this.cropCanvas.height = this.height = this.canvasHeight = this.buffer.height = height;
        if (setImage) {
            this.setImage(this.srcImage);
        }
    }    
    ...
    public customDraw(ctx:CanvasRenderingContext2D, canvas:HTMLCanvasElement):void {
        ...
        if (this.srcImage) {
            ...
            let sourceAspect:number = this.srcImage.height / this.srcImage.width;
            let canvasAspect:number = canvas.height / canvas.width;
            let w:number = canvas.width;
            let h:number = canvas.height;
            if (canvasAspect > sourceAspect) {
                w = canvas.width;
                h = canvas.width * sourceAspect;
            } else {
                h = canvas.height;
                w = canvas.height / sourceAspect;
            }
            this.ratioW = w / this.srcImage.width;
            this.ratioH = h / this.srcImage.height;
            if (canvasAspect < sourceAspect) {
                this.drawImageIOSFix(ctx, this.srcImage, 0, 0, this.srcImage.width, this.srcImage.height,
                    buffer.width / 2 - w / 2, 0, w, h);
            } else {
                this.drawImageIOSFix(ctx, this.srcImage, 0, 0, this.srcImage.width, this.srcImage.height, 0,
                    buffer.height / 2 - h / 2, w, h);
            }
            (<CanvasRenderingContext2D> buffer.getContext('2d'))
            ...
            if (!this.cropperSettings.rounded) {
                ...
                ctx.drawImage(buffer, bounds.left, bounds.top, Math.max(bounds.width, 1),
                    Math.max(bounds.height, 1), bounds.left, bounds.top, bounds.width, bounds.height);
                ctx.strokeRect(bounds.left, bounds.top, bounds.width, bounds.height);
            } else {
                ctx.beginPath();
    ...
    public customSetImage(img:any,canvas:HTMLCanvasElement) {
